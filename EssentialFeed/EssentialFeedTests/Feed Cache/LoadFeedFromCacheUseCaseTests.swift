...
class LoadFeedFromCacheUseCaseTests: XCTestCase {
    ...
    func test_load_deliversCachedImagesOnNonExpiredCache() {
        ...
        expect(sut, toCompleteWith: .success(feed.models), when: {
            store.completeRetrieval(with: feed.locals, timestamp: nonExpiredCache)
        })
    }
    ...
    func test_load_hasNoSideEffectsOnNonExiredCache() {
        let feed = uniqueImageFeed()
        let fixedCurrentDate = Date()
    ...
    private func expect(_ sut: LocalFeedLoader,
        ...
        sut.load { recivedResult in
            ...
            default:
                XCTFail("Expected reuslt \(expectedResult), got \(recivedResult) instead, \(file) \(line)")
            }
